...
**单句不走播放器的播放头。**
Chrome 对 VBR MP3 只能跳到 Xing 索引的格子上，一集 6 分钟的节目格子约 3.7 秒宽：
实测请求 69.88 秒，声音实际从 68.86 秒开始，整句前移一秒，字幕就对不上音了。
所以整集先解码成 PCM，按样点切出每一句再播，误差压到 0.06 秒以内。

**对齐只算一次。**
词级对齐是整条流水线最慢的一步（一集 40 秒）。它在 `prepare` 里算完就落盘，
`build` 直接读，不再跑第二遍。缓存按音频的 sha256 认账——音频对不上就重算，
绝不拿别的音频的时间戳去切句。
