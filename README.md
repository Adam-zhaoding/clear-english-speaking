...
备完课会拿到一个像 `260813_who-does-the-housework.html` 的文件，**双击就能打开**，
播放、变速、快进快退、影子跟读、录音回听全部正常。不需要 WorkBuddy，不需要终端，
不需要起任何服务——这是最简单、最推荐的用法。

### 方式二：在 WorkBuddy 里直接打开

WorkBuddy 的内置浏览器打不开 `file://`，双击给它一个本地路径只会看到一片空白。
想在 WorkBuddy 窗口里听，在发给它的话里加一句 **「帮我打开课件预览」**，
它就会自己起一个本地静态服务，把能点开的链接发给你——你不用记命令，也不用管端口。
...
**只监听 `127.0.0.1:8931`**，同一个局域网里的别人访问不到——课件里内嵌着 BBC 音频，
那是你的个人副本，不该出这台电脑。起好之后打开
`http://127.0.0.1:8931/<期次>_<标题>.html`，播放、跟读、录音全都正常。
